...
print(f"Test set Matthews Correlation Coefficient (MCC): {test_mcc:.4f}")
...
X_train_proj = lda.transform(X_train_resampled)
...
if X_train_proj.shape[1] > 1:
    ...
    for i, label in enumerate(labels):
        plt.scatter(X_train_proj[y_train_resampled == label, 0], X_train_proj[y_train_resampled == label, 1],
    ...
    for i, label in enumerate(labels):
        plt.scatter(X_train_proj[y_train_resampled == label, 0], np.zeros_like(X_train_proj[y_train_resampled == label, 0]),
                    label=f"Class {label}", color=colors[i])
    plt.xlabel("LDA Component 1")
...
n_bootstraps = 20
lda_model = LDA()
stability_results = bootstrap_loocv_stability(X_train_resampled, y_train_resampled, lda_model, n_bootstraps=n_bootstraps)
